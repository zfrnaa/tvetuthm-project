...
import { Dialog, DialogContent, DialogTrigger } from "@/components/ui/dialogs/dialog-radix";
// ShadCN modal
...
export const CardWChart: React.FC<CardProps> = ({ title, content, subtitle, icon, graph, modalContent, modalTitle }) => {
    ...
    <div className="flex flex-col p-4 bg-white/90 dark:bg-cyan-900 backdrop-blur-md rounded-2xl max-w-[400px] justify-between outline">
      <div className="flex flex-row justify-between mb-4 gap-4">
        ...
        <div className="flex flex-row items-center gap-4">
          ...
          <p className="text-lg font-semibold geist-sansSBold" style={{ color: `bg-[e9f1ff] bg-[575757]` }}>
            {title}
          </p>
        </div>
        ...
        {(graph || modalContent) && (
          <Dialog open={modalVisible} onOpenChange={setModalVisible}>
            <DialogTrigger asChild>
              ...
              <button className="w-9 h-9 rounded-full" aria-label="Open Modal">
                <div className="border-2 w-9 h-9 flex justify-center items-center rounded-full" style={{ borderColor: "var(--border-buttonModal)" }}>
                  <MoveDiagonal size={18} color={"var(--icon-diagonal)"} />
                </div>
              </button>
            </DialogTrigger>

            {/* Fullscreen Modal - FIXED */}
            <DialogContent className="flex items-center justify-center bg-opacity-50 backdrop-blur-lg z-50 p-6 border-none md:min-w-3xl !sm:max-w-lg" aria-describedby="showing-chart" aria-description="Showing more details">
              <div className="relative bg-white dark:bg-gray-900 w-full mx-auto rounded-lg shadow-lg p-6 max-h-[90dvh] overflow-y-auto">
                <div className="justify-center">
                  ...
                  <DialogTitle className="text-xl font-semibold text-gray-800 dark:text-white mb-4">{modalTitle || title}</DialogTitle>
                </div>
